...
<article class="column">
  ...
  <div id="frames" class="row wrap">
    {#each topFrames.slice(0, 10) as frameInfo}
      ...
      {@const topCasterUsernames = Object.entries(
        Object.groupBy(
          frameInfo.warpcast_urls.map((url) => url.match(/https:\/\/warpcast.com\/([^/]+)\//)[1]),
          (username) => username,
        ),
      )
            ...
            <span
              >{new Intl.ListFormat('en-US', {
                ...
                topCasterUsernames.length > 2
                  ? [...topCasterUsernames.slice(0, 2), `${topCasterUsernames.length - 2} others`]
                  : topCasterUsernames,
              )}</span>
          </p>
        </div>
